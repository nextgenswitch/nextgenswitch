...
class ExtensionsController extends Controller {
    ...
    public function store( Request $request ) {
        ...
        $organization = Organization::find($orid);
        $total_extension = Extension::where('organization_id', $orid)->count();

        $request->validate([
            'name' => [
                function ($attribute, $value, $fail) use ($organization, $total_extension) {
                    if ($organization->max_extension > 0 && $total_extension >= $organization->max_extension) {
                        $fail('You have reached the maximum number of extensions.');
                    }
                }
            ],
        ]);
