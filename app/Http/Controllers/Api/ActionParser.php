...
class ActionParser {
    ...
    public function parseElement( $elem ) {
       ...
       // info()
        ...
        if ( $tag == 'response' ) {
            ...
            foreach ( $elem->attributes() as $ak => $av ) {
               if ( $ak == 'url' ) {
                    ...
                    $verb['name'] = (string) $av;
                }elseif ( $ak == 'statusCallbackMethod' && in_array( $av, ['POST', 'GET'] ) ) {
        ...
        }elseif ( $tag == 'start' ||  $tag == 'stop'  ||  $tag == 'connect'){
            $stream = [];
            foreach ( $elem as $name => $child ) {
                if ( $name == 'stream' ) {
                    $stream =  $this->parseElement( $child ) ;
                    if ( $tag == 'start' ||  $tag == 'stop') $stream[0]['action'] = ($tag == 'stop')?1:0;
                     if ( $tag == 'connect') $stream[0]['direction'] = 1;
