...
class QueueWorker
{
    ...
    function dial_forward($extension)
    {
        ...
        $dial =  [
            'id' => $extension->id,
            'to' => $extension->forwarding_number,
            "organization_id" => $this->call_queue->organization_id,
            "from" => $caller_id,
            'timeout' => ($this->call_queue->member_timeout > 0) ? $this->call_queue->member_timeout : 30,
    ...
    function getAgentId($active_agents, $sip_user_id)
    {
        foreach ($active_agents as $agent_id => $sip_user) {
            if ($sip_user->id == $sip_user_id) {
                return $agent_id;
            }
        }
        return null;
    ...
    function removeCallFromAgent($active_agents, $agent_id)
    {

        if (isset($active_agents[$agent_id])) {
            $sip_user = $active_agents[$agent_id];
            $sip_user->bridge_call_id = null;
            $sip_user->call_id = null;
            $active_agents[$agent_id] = $sip_user;
            Cache::put('active_agents', $active_agents, 3600); // Cache for 1 hour
        }
    }

    function attachCallToAgent($active_agents, $agent_id, $bridge_call_id, $call_id)
    {

        if (isset($active_agents[$agent_id])) {
            $sip_user = $active_agents[$agent_id];
            $sip_user->bridge_call_id = $bridge_call_id;
            $sip_user->call_id = $call_id;
            $active_agents[$agent_id] = $sip_user;
            Cache::put('active_agents', $active_agents, 3600); // Cache for 1 hour
        }
    }

    function broadcastToWs($bridge_call_id, $call_id = NULL, $event = 'call_bridged')
    {
        info("broadcasting to websocket");
        $bridge_call = Call::find($bridge_call_id);
        if (!$bridge_call) {
            info("bridge call not found");
            return;
        }
        ...
        $agent_id = $this->getAgentId($active_agents, $bridge_call->sip_user_id);
        info("agent id " . $agent_id);

        if (!$agent_id) {
            info("agent not found for sip user " . $bridge_call->sip_user_id);
            return;
        }

        if ($event == 'call_terminated') {
            $this->removeCallFromAgent($active_agents, $agent_id);
            FunctionCall::send_to_websocket($agent_id, ['event' => $event]);
            return;
        }

        if ($event == 'call_bridged' && $call_id) {
            $call = Call::find($call_id);
            if (!$call) return;
            $this->attachCallToAgent($active_agents, $agent_id, $bridge_call->id, $call->id);
            ...
            $data = [
                ...
                'status' => $call->status,
                'duration' => $bridge_call->duration,
                'customer' => Contact::customerLookup($call->caller_id, $call->organization_id),
            ];
            FunctionCall::send_to_websocket($agent_id, ['event' => 'call_bridged', 'data' => $data]);
            return;
        }
    }
    ...
    function process($response)
    {
        ...
        if (request()->query('queue_result') == 1) {  // this is called when agent call finally ended
            ...
            if ($queue) {
                ...
                $this->broadcastToWs($data['call_id'], $queue['call_id']);
            } else {
                $response->leave();
            }
            ...
            $this->broadcastToWs($data['bridge_call_id'], null, 'call_terminated');
