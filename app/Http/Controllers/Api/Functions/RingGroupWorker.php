...
class RingGroupWorker{
    ...
    function send_to_extension($ring_group,$parent_call_id){
        $group_calls = Cache::pull("RingCall:" . $parent_call_id,[]);
        $call_to_send = Cache::pull("RingExtension:" . $parent_call_id,[]);  
    ...
    function send_calls($parent_call_id){
        ...
        $this->send_to_extension($ring_group,$parent_call_id);
        return true;
    ...
    function process($response){   
        ...
        if(request()->query('queue_join') == 1){
            ...
            foreach($group_calls as $k=>$queue_call)
                if($data["call_id"] != $queue_call) { unset($group_calls[$k]); self::hangup($queue_call);}
            ...
            if($key !== false) unset($group_calls[$key]);
            Cache::put("RingCall:" . $parent_call_id,$group_calls,3600);

            if(Cache::get("RingCallStatus:" .$parent_call_id,false) == CallStatusEnum::Established->value) // ring call allready bridged
            return;
