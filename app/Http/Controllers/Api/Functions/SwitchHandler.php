...
class SwitchHandler
{
    ...
    public static function is_ip_blocked($data)
    {
        ...
        if (Cache::has($firewallSettingsKey)) {
            ...
            $settings = Cache::get($firewallSettingsKey);
        ...
        if (! isset($settings['enable_firewall']) || $settings['enable_firewall'] != 1) {
            info("Firewall is disabled");
            return ['blocked' => false, 'count' => 0, 'expire' => 0];
        }


        $ipListKey = 'ipblacklist:list';
        if (Cache::has($ipListKey)) {
            info("Using cached IP blacklist");
            $ipList = Cache::get($ipListKey);
            $ipList = json_decode($ipList, true);
        } else {
            $ipList = IpBlackList::pluck('ip')->toArray();
            Cache::put($ipListKey, json_encode($ipList), 3600);
        }

        if (in_array($ip, $ipList)) {
            return ['blocked' => true, 'count' => 0, 'expire' => $settings['ban_time']];
        }

        $milliseconds = intval(microtime(true) * 1000);

        $count = Redis::eval(
            "redis.call('set','ipblock:$ip:$milliseconds',1) redis.call('expire','ipblock:$ip:$milliseconds',{$settings['find_time']})  return #redis.call('keys', 'ipblock:$ip:*')",
            0
        );
        ...
        if ($count > $settings['failed_attempts_allow']) {
            $blocked = true;
            Setting::saveToTempBlock($ip);
        }

        return ["blocked" => $blocked, 'count' => $count, 'expire' => $settings['ban_time']];
    }
