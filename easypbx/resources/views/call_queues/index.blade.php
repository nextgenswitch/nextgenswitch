...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-heading clearfix">
            ...
            <div class="pull-left">
                <h4 class="tile-title">{{ __('Call Queues') }}</h4>
            </div>

            <div class="btn-group btn-group-sm pull-right" role="group">
                <a href="{{ route('call_queues.call_queue.create') }}" class="btn btn-primary"
                    title="{{ __('Create New Call Queue') }}">
                    <span class="fa fa-plus" aria-hidden="true"></span>{{ __('Create New Call Queue') }}
                </a>
            </div>
            ...
            <div class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
                        ...
                        <div class="btn-group btn-group-sm py-2" role="group"
                            ...
                            <div id="sampleTable_filter" class="dataTables_filter btn-group btn-group-sm">
                                ...
                                <div class="dropdown-menu dropdown-menu-right shadow-dropdown" aria-labelledby="btnFilter">
                                    <a class="dropdown-item" href="{!! route('call_queues.call_queue.index') !!}">{{ __('All') }}</a>
                                    <div class="dropdown-divider"></div>

                                    <a class="dropdown-item"
                                        href="{!! route('call_queues.call_queue.index') !!}?filter=join_empty:1">{{ __('Join Empty') }}</a>
                                    <div class="dropdown-divider"></div>

                                    <a class="dropdown-item"
                                        href="{!! route('call_queues.call_queue.index') !!}?filter=leave_when_empty:1">{{ __('Leave when empty') }}</a>
                                    <div class="dropdown-divider"></div>

                                    <a class="dropdown-item"
                                        href="{!! route('call_queues.call_queue.index') !!}?filter=queue_callback:1">{{ __('Queue Callback') }}</a>
                                    <div class="dropdown-divider"></div>

                                    <a class="dropdown-item"
                                        href="{!! route('call_queues.call_queue.index') !!}?filter=record:1">{{ __('Record') }}</a>
                ...
                <div class="table-responsive">
                    <div id="crud_contents">
                        @include ('call_queues.table', ['callQueues' => $callQueues])
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" action="{!! route('call_queues.call_queue.bulk') !!}" class="editableForm" id="massActionFrm"
    ...
    <div class="modal fade drawer right-align" id="FormModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('Create New Call Queue') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
