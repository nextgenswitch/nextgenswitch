...
{!! Form::text('name',old('name', optional($mailProfile)->name), ['class' => 'form-control' . ($errors->has('name') ? ' is-invalid' : null), 'minlength' => '1', 'maxlength' => '191', 'required' => true, 'placeholder' => __('Enter name here...'), ]) !!}
    ...
    <div class="form-group @error('provider') has-error @enderror">
        ...
        <div class="input-group">    
            ...
            <select name="provider" id="provider" name="provider" class="form-control">
                ...
                @foreach (config('enums.mail_providers') as $key => $provider)
                    <option @selected($key == old('provider', optional($mailProfile)->provider)) value="{{ $key }}"> {{ $provider['name'] }} </option>    
                @endforeach
...
{{-- <div class="col-lg-12">
    <div class="form-group @error('options') has-error @enderror">
        ...
        <span class="text-required">*</span>
            ...
            {!! Form::textarea('options', old('options', optional($mailProfile)->options), ['class' => 'form-control', 'required' => true, 'placeholder' => __('Enter options here...'), ]) !!}
...
<div class="form-group @error('organization_id') has-error @enderror">
    ...
    <span class="text-required">*</span>
        ...
        {!! Form::select('organization_id',$organizations,old('organization_id', optional($mailProfile)->organization_id), ['class' => 'form-control', 'required' => true, 'placeholder' => __('Select organization'), ]) !!}
...
<div class="col-lg-6">
    <div class="form-group @error('status') has-error @enderror">
        {!! Form::label('status', __('Active?'),['class' => 'control-label']) !!}
            ...
            <div class="checkbox">
                <label for='status'>
                    {!! Form::checkbox('status', '1',  (old('status', optional($mailProfile)->status) == '1' ? true : null) , ['id' => 'status', 'class' => ''  . ($errors->has('status') ? ' is-invalid' : null), ]) !!}
                    {{ __('Yes') }}
                </label>
            </div>
    ...
    <div class="col-lg-12">
        <div class="form-group @error('default') has-error @enderror">
            {!! Form::label('default', __('Default ?'),['class' => 'control-label']) !!}
                ...
                <div class="checkbox">
                    <label for='default'>
                        {!! Form::checkbox('default', '1',  (old('default', (isset($mailProfile->default) && $mailProfile->default == 1) ? true : null)) , ['id' => 'default', 'class' => ''  . ($errors->has('default') ? ' is-invalid' : null), ]) !!}
                        {{ __('Yes') }}
                    </label>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        var options = JSON.parse(@json(optional($mailProfile)->options));

        if(options === null){
            options = @json(config('enums.mail_providers.Smtp.options'));
            placeholder = true;
        }
