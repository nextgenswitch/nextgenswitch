...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-heading clearfix">
            ...
            <div class="pull-left">
                <h4 class="tile-title">{{ __('AI Providers') }}</h4>
            </div>

            <div class="btn-group btn-group-sm pull-right" role="group">

                <a data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" href="#"
                    class="btn btn-primary dropdown-toggle" title="{{ __('Create New LLM Profile') }}">
                    <span class="fa fa-plus" aria-hidden="true"></span> {{ __('Create New LLM Profile') }}
                </a>
                <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                    <a class="dropdown-item btnForm "
                        href="{{ route('tts_profiles.tts_profile.create', 2) }}">{{ __('Create New LLM Profile') }}</a>
                    <a class="dropdown-item btnForm "
                        href="{{ route('tts_profiles.tts_profile.create', 0) }}">{{ __('Create New Tts Profile') }}</a>
                    <a class="dropdown-item btnForm "
                        href="{{ route('tts_profiles.tts_profile.create', 1) }}">{{ __('Create New STT Profile') }}</a>
                </div>
            </div>
                        ...
                        <div class="btn-group btn-group-sm py-2" role="group"
                            ...
                            <div id="sampleTable_filter" class="dataTables_filter btn-group btn-group-sm">
                                ...
                                <div class="dropdown-menu shadow-dropdown dropdown-menu-right" aria-labelledby="btnFilter">
                                    ...
                                    <a class="dropdown-item" href="{!! route('tts_profiles.tts_profile.index',['filter'=>'type:2']) !!}">{{ __('LLM Profiles') }}</a>
    ...
    <div class="modal fade drawer right-align" id="FormModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('Create New AI Provider') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
