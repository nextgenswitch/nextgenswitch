...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-heading clearfix">
            ...
            <div class="pull-left">
                <h4 class="tile-title">{{ __('Extensions') }}</h4>
            </div>

            <form method="POST" enctype="multipart/form-data" action="{!! route('extensions.extension.import') !!}">
                @csrf
                <div class="btn-group btn-group-sm pull-right" role="group">
                    ...
                    &nbsp;
                    <a href="{{ route('extensions.extension.create') }}" class="btn btn-primary btnForm"
                        title="{{ __('Create New Extension') }}">
                        <span class="fa fa-plus" aria-hidden="true"></span>{{ __('Create New Extension') }}
                    </a>
                </div>
            </form>
            ...
            <div class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
                        ...
                        <div class="btn-group btn-group-sm py-2" role="group"
                            ...
                            <div id="sampleTable_filter" class="dataTables_filter btn-group btn-group-sm">
                                ...
                                <div class="dropdown-menu dropdown-menu-right shadow-dropdown" aria-labelledby="btnFilter">
                                    <a class="dropdown-item" href="{!! route('extensions.extension.index') !!}">{{ __('All') }}</a>
                                    <div class="dropdown-divider"></div>
                                    <a class="dropdown-item"
                                        href="{!! route('extensions.extension.index') !!}?filter=status:1">{{ __('Active') }}</a>
                                    <div class="dropdown-divider"></div>
                                    <a class="dropdown-item"
                                        href="{!! route('extensions.extension.index') !!}?filter=status:0">{{ __('Disabled') }}</a>
                                </div>
                            </div>
                ...
                <div class="table-responsive">
                    <div id="crud_contents">
                        @include ('extensions.table', ['extensions' => $extensions])
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" action="{!! route('extensions.extension.bulk') !!}" class="editableForm" id="massActionFrm"
    ...
    <div class="modal fade drawer right-align" id="FormModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('Create New Extension') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
