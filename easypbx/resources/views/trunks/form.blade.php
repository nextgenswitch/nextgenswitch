...
<div class="row">
    <div class="col-lg-12">
    <div class="form-group @error('overwrite_cid') has-error @enderror">
        {!! Form::label('overwrite_cid', __('Overwrite CID ?'), ['class' => 'control-label']) !!}
        

        <div class="checkbox">
            <label for='overwrite_cid'>
                {!! Form::checkbox('overwrite_cid', '1', old('overwrite_cid', isset($sip->overwrite_cid) && $sip->overwrite_cid == 1 ? true : null), [
                    'id' => 'overwrite_cid',
                    'class' => '' . ($errors->has('overwrite_cid') ? ' is-invalid' : null),
                ]) !!}
                {{ __('Yes') }}
            </label>
        </div>
