...
<div class="form-group @error('voice_id') has-error @enderror">
    ...
    <div class="col-lg-12 @if (optional($survey)->type == 1 || optional($survey)->type == 0) d-none @endif" id="inent_analyzer_div">
        <div class="form-group @error('intent_analyzer') has-error @enderror">
            ...
            {!! Form::textarea('intent_analyzer', old('intent_analyzer', optional($survey)->intent_analyzer), [
        ...
        <div class="card">
            ...
            <table class="table dynamicForm ajaxForm" id="dynamicFormTable">
                <tr>
                    ...
                    <td style="border-top: none;"> {!! Form::label('module', __('Module'), ['class' => 'control-label']) !!} </td>
                    <td style="border-top: none;"> {!! Form::label('destination', __('Destination'), ['class' => 'control-label']) !!} </td>
                    <td style="border-top: none;"> {!! Form::label('action', __('Voice Matchable Intent/Text'), ['class' => 'control-label']) !!} </td>
                </tr>
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '#type', function(event) {
                ...
                if (typeVal == 0 || typeVal == 1) {
                    $("#inent_analyzer_div").hide();
                } else {
                    $("#inent_analyzer_div").show();
                }
            ...
            if ($("#edit_survey_form").length > 0) {
                ...
                if (keys.length > 0) {
                    $.each(keys, function(index, item) {
                        ...
                        $.each(item, (idx, itm) => {
                            console.log("test", idx, itm)

                            $("#dynamicFormTable tr:last").find('.key').val(item.key);
                            $("#dynamicFormTable tr:last").find('.intent').val(item.intent);
                            $("#dynamicFormTable tr:last").find('.text').val(item.text);
                            $("#dynamicFormTable tr:last").find('.function_id').val(getFucID(item
                                .function_id));
                            $("#dynamicFormTable tr:last").find('.destination_id').html(item
                                .destinations);
                            $("#dynamicFormTable tr:last").find('.destination_id').val(item
                                .destination_id)
                        })
            ...
            function generateRow(edit = 0) {
                ...
                row +=
                    '<td> <input class="form-control intent" placeholder="Enter intent/text here..." name="keys[intent][]" type="text"></td>';
