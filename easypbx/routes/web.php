...
Route::group(['middleware' => 'auth'], function () {
    ...
    Route::group(['middleware' => 'permission:admin.monitoring.*'], function () {
        Route::group([
            ...
            Route::post('/summarize-conversation', [MonitoringController::class, 'summarizeConversation'])
                ->name('monitoring.conversation.summarize');
    ...
    Route::group([
        ...
        Route::get('/', [AiBotsController::class, 'index'])
            ->name('ai_bots.ai_bot.index');
        Route::get('/create', [AiBotsController::class, 'create'])
            ->name('ai_bots.ai_bot.create');
        Route::get('/show/{aiBot}', [AiBotsController::class, 'show'])
            ->name('ai_bots.ai_bot.show')->where('id', '[0-9]+');
        Route::get('/{aiBot}/edit', [AiBotsController::class, 'edit'])
            ->name('ai_bots.ai_bot.edit')->where('id', '[0-9]+');
        Route::post('/', [AiBotsController::class, 'store'])
            ->name('ai_bots.ai_bot.store');
        Route::put('ai_bot/{aiBot}', [AiBotsController::class, 'update'])
            ->name('ai_bots.ai_bot.update')->where('id', '[0-9]+');
        Route::delete('/ai_bot/{aiBot}', [AiBotsController::class, 'destroy'])
            ->name('ai_bots.ai_bot.destroy')->where('id', '[0-9]+');
        Route::put('field/{aiBot}', [AiBotsController::class, 'updateField'])
            ->name('ai_bots.ai_bot.updateField');
        Route::put('/bulk', [AiBotsController::class, 'bulkAction'])
            ->name('ai_bots.ai_bot.bulk');

        Route::get('distinations/{function}', [AiBotsController::class, 'destinations'])
            ->name('ai_bots.ai_bot.destinations');

        Route::get('/ai_assistant_calls', [AiBotsController::class, 'assistantCalls'])->name('ai_bots.ai_bot.ai_assistant_calls');
    ...
    Route::group([
        ...
        Route::get('/', [DashboardController::class, 'indexNew'])->name('dashboard.index');
        Route::get('/extension-stats', [DashboardController::class, 'extensionStats'])->name('dashboard.extension.stats');
        Route::get('/call-stats', [DashboardController::class, 'callStats'])->name('dashboard.call.stats');
        Route::get('/trunk-stats', [DashboardController::class, 'trunkStats'])->name('dashboard.trunk.stats');
        Route::get('/queue-stats', [DashboardController::class, 'queueStats'])->name('dashboard.queue.stats');
        Route::get('/bridge-call-statistrics', [DashboardController::class, 'bridgeCallStats'])->name('dashboard.bridge.call.stats');
    });
    ...
    Route::group(['middleware' => 'permission:admin.*'], function () {
        ...
        Route::group([
            ...
            Route::get('/voice_files/download/{path}', [VoiceFilesController::class, 'download'])
                ->name('voice_files.voice.download');
        ...
        Route::get('test', function () {
            ...
            // return storage_path('test.mp3');
            $res = App\Tts\Tts::speechToText(storage_path('test.mp3'), auth()->user()->organization_id, 6);

            $text = $res['text'];
            print_r($text);

            $prompt = "You are a helpful assistant. Please summarize the following text: $text";
            $instruction = "always provide the output as a dialoge format customer and agent. provide as pre html format";

            $res = App\Tts\Tts::llm($prompt, $instruction, auth()->user()->organization_id, 3);
            print_r($res);
            die();

            // $response = Http::asForm()->post("http://192.168.88.211:5000/agent", ['prompt' => 'test']);
            // return $response;

            // $organization = Organization::find(auth()->user()->organization_id);

            // dd(Carbon::parse($organization->expire_date)->isPast());
    ...
    Route::group([
        ...
        Route::get('/assign/me/{ticket_id}', [TicketsController::class, 'assignMe'])->name('tickets.ticket.assign');
