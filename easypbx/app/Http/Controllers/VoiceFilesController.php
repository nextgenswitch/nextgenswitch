...
class VoiceFilesController extends Controller {
    ...
    {
        if (is_numeric($path)) {
            $voice = VoiceFile::find($path);
            if ($voice) {
                $filePath = storage_path('app/public/uploads/' . auth()->user()->organization_id . '/' . $voice->file_name);
            }
    ...
    protected function getData( Request $request ) {
        ...
        if (  ! $request->hasFile( 'file' ) ) {
            ...
            $rules['file'] = 'required|mimes:mp3,wav|max:20480';
        }
